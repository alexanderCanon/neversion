...
| 2026-05-11 | Infra / Dokploy store | Se agregó `apps/store/compose.prod.yml` para desplegar la tienda SSR desde Dokploy usando build context en la raíz del monorepo y variables runtime `STORE_API_URL`, `STORE_SUPABASE_URL`, `STORE_SUPABASE_KEY`, `STORE_VENDOR_UUID`. | Documentado en `docs/implementation/dokploy-frontend-deploy.md`. |
| 2026-05-11 | Store QA / Platforms build | Corrección de conflictos en `PlatformsComponent`: template HTML reconstruido con estructura válida, eliminación de método `resolveImageUrl` duplicado e inyección de `ToastService` para el feedback de carrito. | Verificación local: `pnpm run build:ssr` PASS con warning preexistente de budget inicial y selector Bootstrap. |
| 2026-05-11 | Infra / CI store | `ci-store.yml` dejó de usar npm y `apps/store/package-lock.json`; ahora usa pnpm con cache sobre `pnpm-lock.yaml`, instala desde la raíz del workspace y ejecuta `pnpm --filter neversion-site build:ssr`. | Corrige el fallo de GitHub Actions `Some specified paths were not resolved` causado por un lockfile inexistente. |
| 2026-05-12 | Infra / Security hardening | El servidor SSR de store ahora agrega headers defensivos (`nosniff`, `DENY`, referrer policy, permissions policy, HSTS y CSP parcial no disruptiva). Links externos en pestañas nuevas usan `rel="noopener noreferrer"` e `ImageService` rechaza URLs no HTTPS o esquemas no esperados. | Verificación local: `pnpm run build:ssr` PASS con warning preexistente de budget inicial y selector Bootstrap. |
| 2026-05-12 | Infra / Dokploy store resources | `apps/store/compose.prod.yml` define límite inicial de memoria `512m` para el contenedor SSR Node en Dokploy. | Límite conservador para MVP; subir a `768m`/`1g` si Dokploy reporta OOM durante runtime. |
